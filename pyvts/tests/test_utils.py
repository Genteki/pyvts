    ...
    async def handler(self, websocket):
        ...
        while True:
            ...
            # About Custom Parameters
                ...
                if pname in self.custom_param:
                    self.custom_param[pname]["value"] = (float(dict_msg["data"]["parameterValues"][0]["value"]) * 
                                                         float(dict_msg["data"]["parameterValues"][0]["weight"]) + 
                                                         float(self.custom_param[pname]["value"]) * 
                                                         float((1-dict_msg["data"]["parameterValues"][0]["weight"])))
                    send_msg["data"] =  {
                        'parameter': pname, 
                        'message': 'set'
                    }
